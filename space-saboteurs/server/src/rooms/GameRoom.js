...
class GameRoom {
  ...
  addPlayer({ name, socketId }) {
    ...
    const player = {
      ...
      color:
        PLAYER_COLORS[
          this._nextColorIdx % PLAYER_COLORS.length
        ],
  ...
  updateSettings(playerId, settings) {
    ...
    if (Number.isInteger(settings.imposters)) {
      s.imposters = Math.max(
        1,
        Math.min(3, settings.imposters)
      );
    }

    if (Number.isInteger(settings.killCooldownMs)) {
      s.killCooldownMs = Math.max(
        5000,
        Math.min(60000, settings.killCooldownMs)
      );
    }

    if (Number.isInteger(settings.taskCount)) {
      s.taskCount = Math.max(
        2,
        Math.min(
  ...
  canStart(playerId) {
    ...
    if (this.settings.imposters >= connectedCount) {
      return { error: "TOO_MANY_IMPOSTERS" };
    }
  ...
  assignTasks() {
    const pool = [...TASK_DEFINITIONS]
      .sort(() => Math.random() - 0.5)
      .slice(0, this.settings.taskCount);
  ...
  completeTask(playerId, taskId) {
    ...
    const def = TASK_DEFINITIONS.find(
      (t) => t.id === taskId
    );
  ...
  reportBody(playerId, bodyId) {
    ...
    this.startMeeting(
      reporter,
      `${reporter.name} reported ${body.victimName}'s body.`
    );
  ...
  submitVote(playerId, targetId) {
    ...
    if (
      this.meeting.votes.has(playerId)
    ) {
      return {
        error: "ALREADY_VOTED",
      };
  ...
  submitChat(playerId, text) {
    ...
    if (this.meeting.chat.length > 100) {
      this.meeting.chat.shift();
    }
  ...
  repairSabotage(playerId, roomName) {
    ...
    const playerRoom =
      roomAt(player.x, player.y);

    if (
      !def.fixRooms.includes(
        playerRoom
      )
    ) {
      return {
    ...
    this.sabotage.fixedRooms.add(
      playerRoom
    );

    const complete =
      def.fixRooms.every((r) =>
        this.sabotage.fixedRooms.has(r)
      );
  ...
  checkWinConditions() {
    ...
    if (
      aliveImposters.length >=
      aliveCrew.length
    ) {
      return this.endGame(
        "IMPOSTERS",
        "Imposters equal or outnumber the crew."
      );
    }
    ...
    if (this.sabotage) {
      ...
      const elapsed =
        Date.now() -
        this.sabotage.startedAt;

      const isFatal =
        this.sabotage.type === "oxygen" ||
        this.sabotage.type === "reactor";
      ...
      if (
        ...
        return this.endGame(
          "IMPOSTERS",
          `${def.label} sabotage was not fixed in time.`
        );
      }
    }
  ...
  tick() {
    ...
    if (
      ...
      const allVoted =
        this.meeting.votes.size >=
        aliveCount;

      if (
        elapsed >=
          this.meeting.durationMs ||
        allVoted
      ) {
        const result =
          this.resolveMeeting();
        ...
        this.io
          ...
          .emit(
            ...
            result && {
              ejectedName:
                result.ejectedPlayer
                  ? result.ejectedPlayer.name
                  : null,

              ejectedRole:
                result.ejectedPlayer
                  ? result.ejectedPlayer.role
                  : null,
    ...
    return {
      ...
      sabotage: this.sabotage
        ? {
            ...
            timerMs:
              this.sabotage.timerMs,

            fixedRooms: [
              ...this.sabotage.fixedRooms,
            ],
